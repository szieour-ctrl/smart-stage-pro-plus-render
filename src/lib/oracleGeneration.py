...
The actual Flux Kontext call that was the missing piece for an entire
prior session -- every winning test up to now used Oracle images Sam
generated externally (fal.ai playground / hand-upload) rather than a
live call from this module. This is what produces them live, in the
pipeline, from a given Original photo.

CRITICAL FIX (this session): this module previously called OpenAI's
gpt-image-2 (images.edit). That was never correct -- every piece of
winning evidence collected against this pipeline (MLS Bright interior
chair-detail recovery, clean exterior alignment at 2847/2857 inliers,
do-no-harm behavior on already-bright photos) came from Flux Kontext
renders, not GPT Image 2 ones. This module now calls fal.ai's Flux
Kontext [pro] endpoint (fal-ai/flux-pro/kontext) instead. See git
history / the prior module docstring if you need the old OpenAI code
for reference -- it should not be resurrected without new evidence.

ENDPOINT CHOICE: fal-ai/flux-pro/kontext is Black Forest Labs' Kontext
[pro] model, an image-EDITING endpoint (takes an existing image + a
prompt describing the change), not a text-to-image endpoint -- the
correct category for a correction task, same reasoning that picked
images.edit over images.generate on the OpenAI side originally.

QUEUE PATTERN, NOT A SINGLE SYNCHRONOUS POST: per the handoff, this
follows the same fal.ai queue submit -> poll -> result pattern already
established and battle-tested in this codebase's video pipeline
(klingMotion.js, ltxMotion.js) rather than inventing a second fal.ai
integration style. Both of those chose explicit queue.submit() +
polling over fal.subscribe()'s blocking internal poll specifically
because a silent stall in the SDK's internal loop was confirmed twice
in testing with zero JS-catchable error -- the same risk applies here,
so this module talks to fal.ai's plain REST queue endpoints directly
(https://queue.fal.run/...) rather than depending on the fal_client
Python SDK, which is ALSO not in requirements.txt. Flux Kontext's
measured latency (3-8s in prior testing) is far faster than Kling/LTX
video jobs, so polling here uses a shorter interval and a shorter
ceiling than the video pipeline's 10s/15min -- see POLL_INTERVAL_SECONDS
and MAX_POLL_ATTEMPTS below. Revisit those two constants specifically
if a real batch shows Flux Kontext running slower than 3-8s in
production.
...
as a real incident there). Flux Kontext's image input is a plain string
field (image_url, which fal.ai accepts as either a hosted URL or a
base64 data URI -- see fal.ai's own docs on this) inside a JSON body,
so this module no longer needs the hand-built multipart encoder the
OpenAI version required; a data URI is built directly from the raw
image bytes instead.
...
TIMEOUT AND RETRY: unlike OpenAI's ~2-minute guidance this module was
originally sized around, Flux Kontext's measured latency this session
was 3-8s -- a materially different profile that does not need the same
retry posture. Retries here are fewer, with a shorter base backoff, but
still use jitter -- the reason for jitter (a fleet of Railway workers
retrying a shared rate-limit hit in lockstep) doesn't depend on which
API is on the other end. If a real production batch shows Flux Kontext
running slower or less reliably than this session's measurements,
MAX_RETRIES / BASE_BACKOFF_SECONDS / MAX_POLL_ATTEMPTS are the knobs to
revisit, not a reason to silently fall back to OpenAI's old numbers.

FAL_KEY, NOT OPENAI_API_KEY: this is the actual gotcha carried over
from the prior module -- the OpenAI key was confirmed added to Railway
in an earlier session, but that's the wrong key for this endpoint.
fal.ai's standard auth convention is FAL_KEY (Authorization: Key
$FAL_KEY, not Bearer). Confirm FAL_KEY is actually set on Railway --
per the handoff, this was flagged as unconfirmed, not just a naming
tweak to make in code. If generate_oracle_scene() reports
missing_FAL_KEY despite believing a fal key was added, check the exact
Railway variable name matches FAL_KEY before assuming anything else.
...
# docstring's "FAL_KEY, NOT OPENAI_API_KEY" section: this is flagged as
# unconfirmed against actual Railway config, not just a code-level rename.
...
# below. 30s is generous for a control call; if one of these hangs that
# long something is genuinely wrong with the network, not with Flux.
...
def generate_oracle_scene(orig_image_bytes: bytes, is_exterior: bool) -> tuple:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            # fal.ai queue FAILED status, poll timeout, bad response shape
            last_error = f"{type(e).__name__}: {e}"
            if attempt == MAX_RETRIES:
                break

        # Exponential backoff with jitter -- shorter base than the old
        # OpenAI-sized version (see BASE_BACKOFF_SECONDS above), but the
        # jitter itself still matters for the same reason: multiple
        # Railway workers retrying a shared rate-limit hit in lockstep
        # would otherwise all land on the limiter at the same instant.
